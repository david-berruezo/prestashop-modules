...
class ClassMyreasurance extends ObjectModel
{
    public $id_tab;
    public $id_shop;
    public $title;
    ...
    public function delete()
    {
        ...
        if ($images) {
            foreach ($images as $image) {
                if ($image && file_exists(_PS_MODULE_DIR_.'myreasurance/images/'.$image)) {
                    $res &= @unlink(_PS_MODULE_DIR_.'myreasurance/images/'.$image);
                }
            }
        }
    ...
    public static function getSlickList()
    {
        $sql = 'SELECT bons.*, bonsl.`title`, bonsl.`url`, bonsl.`image`
                FROM '._DB_PREFIX_.'myreasurance bons
                JOIN '._DB_PREFIX_.'myreasurance_lang bonsl
