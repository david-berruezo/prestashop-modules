...
include_once(_PS_MODULE_DIR_.'myreasurance/classes/ClassMyreasurance.php');

class Myreasurance extends Module
{
    protected $config_form = false;

    public function __construct()
    {
        $this->name = 'myreasurance';
        ...
        $this->displayName = $this->l('My reasurance');
        $this->description = $this->l('Display awesome reasuramce');
    ...
    public function createAjaxController()
    {
        ...
        if (is_array($languages)) {
            foreach ($languages as $language) {
                $tab->name[$language['id_lang']] = 'myreasurance';
            }
        }

        $tab->class_name = 'AdminAjaxMyreasurance';
    ...
    {
        if ($tab_id = (int)Tab::getIdFromClassName('AdminAjaxMyreasurance')) {
            $tab = new Tab($tab_id);
            $tab->delete();
        }
    ...
    {
        $settings = array(
            'MY_SLICK_CAROUSEL_LOOP' => true,
            'MY_SLICK_CAROUSEL_NAV' => true,
            'MY_SLICK_CAROUSEL_DOTS' => true,
            'MY_SLICK_CAROUSEL_DRAG' => true,
            'MY_SLICK_CAROUSEL_AUTOPLAY' => true,
            'MY_SLICK_CAROUSEL_TIME' => 5000,
        );
        return $settings;
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::getIsset('updatebonslick') || Tools::getValue('updatebonslick')) {
            $output .= $this->renderSlickForm();
        } elseif ((bool)Tools::isSubmit('addmyreasurance')) {
            $output .= $this->renderSlickForm();
        } elseif (!$result) {
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'name' => 'MY_SLICK_CAROUSEL_AUTOPLAY',
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    protected function validateSettings()
    {
        ...
        if (Tools::isEmpty(Tools::getValue('MY_SLICK_CAROUSEL_TIME'))) {
            $errors[] = $this->l('Animation speed is required.');
        } else {
            if (!Validate::isUnsignedInt(Tools::getValue('MY_SLICK_CAROUSEL_TIME'))) {
    ...
    protected function renderSlickForm()
    {
        ...
        if ((bool)Tools::getIsset('updatebonslick') && (int)Tools::getValue('id_tab') > 0) {
            $tab = new ClassMyreasurance((int)Tools::getValue('id_tab'));
    ...
    {
        if (!$tabs = ClassMyreasurance::getSlickList()) {
            $tabs = array();
        }
    ...
    protected function addSlick()
    {
        ...
        if ((int)Tools::getValue('id_tab') > 0) {
            $item = new ClassMyreasurance((int)Tools::getValue('id_tab'));
        } else {
            $item = new ClassMyreasurance();
        }
    ...
    protected function preValidateForm()
    {
        ...
        $class = new ClassMyreasurance((int)Tools::getValue('id_tab'));
    ...
    {
        $tab = new ClassMyreasurance(Tools::getValue('id_tab'));
        $res = $tab->delete();
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        Media::addJsDefL('ajax_theme_url', $this->context->link->getAdminLink('AdminAjaxMyreasurance'));
        $this->context->smarty->assign('ajax_theme_url', $this->context->link->getAdminLink('AdminAjaxMyreasurance'));
    ...
    public function hookDisplayMyCustomReaseguros(){
        ...
        $slider_front = new ClassMyreasurance();
        $tabs = $slider_front->getTopFrontItems($this->id_shop, true);
        $result = array();
        ...
        return $this->display(__FILE__, 'views/templates/hook/displayMyCustomReasurance.tpl');
    }
