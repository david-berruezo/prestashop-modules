...
include_once(_PS_MODULE_DIR_.'myslider/classes/ClassMyslider.php');

class Myslider extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('My slider');
        $this->description = $this->l('Display awesome image slider');
    ...
    public function createAjaxController()
    {
        ...
        if (is_array($languages)) {
            foreach ($languages as $language) {
                $tab->name[$language['id_lang']] = 'myslider';
            }
        }

        $tab->class_name = 'AdminAjaxMyslider';
    ...
    {
        if ($tab_id = (int)Tab::getIdFromClassName('AdminAjaxMyslider')) {
            $tab = new Tab($tab_id);
            $tab->delete();
        }
    ...
    public function getContent()
    {
        ...
        if (Tools::getIsset('updatebonslick') || Tools::getValue('updatebonslick')) {
            $output .= $this->renderSlickForm();
        } elseif ((bool)Tools::isSubmit('addmyslider')) {
            $output .= $this->renderSlickForm();
        } elseif (!$result) {
    ...
    protected function renderSlickForm()
    {
        ...
        if ((bool)Tools::getIsset('updatebonslick') && (int)Tools::getValue('id_tab') > 0) {
            $tab = new ClassMyslider((int)Tools::getValue('id_tab'));
    ...
    {
        if (!$tabs = ClassMyslider::getSlickList()) {
            $tabs = array();
        }
    ...
    protected function addSlick()
    {
        ...
        if ((int)Tools::getValue('id_tab') > 0) {
            $item = new ClassMyslider((int)Tools::getValue('id_tab'));
        } else {
            $item = new ClassMyslider();
        }
    ...
    protected function preValidateForm()
    {
        ...
        $class = new ClassMyslider((int)Tools::getValue('id_tab'));
    ...
    {
        $tab = new ClassMyslider(Tools::getValue('id_tab'));
        $res = $tab->delete();
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        Media::addJsDefL('ajax_theme_url', $this->context->link->getAdminLink('AdminAjaxMyslider'));
        $this->context->smarty->assign('ajax_theme_url', $this->context->link->getAdminLink('AdminAjaxMyslider'));
    ...
    public function hookDisplayMyCustomBanner(){
        ...
        $slider_front = new ClassMyslider();
        $tabs = $slider_front->getTopFrontItems($this->id_shop, true);
        $result = array();
